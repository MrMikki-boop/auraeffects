// ============================================================
// ПЕПЕРБОКС — Foundry v13 (PERFECT MISFIRE LOGIC)
...
let ashUsedThisTurn = false; // Контроль Пепла (1 раз за ход)
...
// ============================================================
// 🎲 ROLL → toMessage (Dice So Nice Sync)
// ============================================================
  ...
  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
    ...
    <select class="shot-select" data-index="${index}"
      ...
      style="background:#111; color:${dist > RANGE_MAX ? "#666" : "#fff"};
             border:1px solid #8c5a2b; border-radius:6px; height:28px; width:50px; text-align:center;">
...
<div id="roll-mode-group" style="display:flex; gap:6px; margin-bottom:12px;">
  <label id="lbl-adv" style="flex:1; display:flex; flex-direction:column; align-items:center; gap:3px; padding:7px 4px; border-radius:8px; cursor:pointer; border:1px solid #3a2010; background:rgba(0,0,0,0.3); font-size:12px; color:#a08060;">
    <input type="radio" name="roll-mode" value="advantage" style="display:none;"><span>🎯</span><span>Преимущество</span>
  </label>
  <label id="lbl-normal" style="flex:1; display:flex; flex-direction:column; align-items:center; gap:3px; padding:7px 4px; border-radius:8px; cursor:pointer; border:1px solid #f4a235; background:rgba(244,162,53,0.12); font-size:12px; color:#f4a235;">
    <input type="radio" name="roll-mode" value="normal" checked style="display:none;"><span>🎲</span><span>Обычный</span>
  </label>
  <label id="lbl-dis" style="flex:1; display:flex; flex-direction:column; align-items:center; gap:3px; padding:7px 4px; border-radius:8px; cursor:pointer; border:1px solid #3a2010; background:rgba(0,0,0,0.3); font-size:12px; color:#a08060;">
    <input type="radio" name="roll-mode" value="disadvantage" style="display:none;"><span>⚠️</span><span>Помеха</span>
  </label>
</div>`;

const content = `
<div style="background:linear-gradient(145deg,#0f0f0f,#1a1208); padding:15px; border-radius:12px; border:1px solid #5a3a1a; color:#f1f3f5;">
  <div style="border-bottom:1px solid #444; margin-bottom:10px; padding-bottom:5px;">
    <b>Выстрелов осталось:</b> <span id="shots-left" style="color:#ff922b; font-size:16px; font-weight:bold;">${MAX_SHOTS}</span>
  </div>
  ${rollModeHtml}${targetsHtml}
</div>`;
...
const dialogResult = await new Promise((resolve) => {
        ...
        _onRender() {
            ...
            const updateShots = () => {
                ...
                selects.forEach(sel => {
                    ...
                    for (const opt of sel.options) opt.disabled = Number(opt.value) > cur + rem;
                });
            };
            ...
            el.querySelectorAll("input[name='roll-mode']").forEach(inp => inp.addEventListener("change", updateMode));
            updateMode();
        }
    ...
    new PepperboxDialog({
        window: {title: "Пепербокс"}, content,
        buttons: [{
            action: "fire", label: "🔥 Огонь!", callback: (e, b, dialog) => {
...
if (lastUse !== turnId) {
    useAshBarrel = await confirm("Пепел в стволе", "<p>🔥 Использовать <b>Пепел в стволе</b>?</p>");
...
outer:
    for (const entry of selections) {
        for (let i = 0; i < entry.shots; i++) {
            ...
            if (dist > RANGE_MAX) {
                ChatMessage.create({content: `<div style="background:#1a1a1a; border:1px solid #ff4d4f; padding:8px; border-radius:8px; color:#f1f3f5;">⛔ <b>${entry.target.name}</b> вне досягаемости (${dist} фт).</div>`});
                continue;
            }

            let shotMode = disadvantageNext ? "disadvantage" : chosenMode;
            if (dist > RANGE_NORMAL) shotMode = shotMode === "advantage" ? "normal" : "disadvantage";
            disadvantageNext = false;

            const rollBase = shotMode === "advantage" ? "2d20kh1" : shotMode === "disadvantage" ? "2d20kl1" : "1d20";
            ...
            if (typeof Sequence !== "undefined") {
                new Sequence().sound().file("blfx.sound.weapon.range.pistol_shot1.1").volume(0.9).delay(100)
                    .effect().file("jb2a.bullet.Snipe.orange").atLocation(token).stretchTo(entry.target).play();
            }
            ...
            // ==========================================================
            if (d20 <= MISFIRE) {
                const accept = await confirm("💥 Осечка!", `<p style="color:#f1f3f5;">💥 Произошла осечка (выпало <b>${d20}</b>).<br>Принять последствия?<br><span style="font-size:11px; color:#aaa;">(Отмена = переброс за вдохновение)</span></p>`);

                if (!accept) {
                    const reroll = await rollAndSend(formula, actor.getRollData(), "🎯 Переброс (вдохновение)");
                    const newD20Term = reroll.dice?.find(d => d.faces === 20);
                    const newD20 = newD20Term?.results.find(r => r.active)?.result ?? newD20Term?.results[0]?.result;
                    const isCrit = newD20 >= critThreshold;
                    const ac = entry.target.actor.system.attributes.ac.value;

                    await processHit(reroll.total >= ac || isCrit, isCrit, entry.target, useAshBarrel, false);
                    continue;
                }
                ...
                const mishapTable = {
                    1: {
                        icon: "💣",
                        title: "Взрыв в стволе",
                        color: "#ff4d4f",
                        desc: "Вы получаете урон оружия. Оружие ломается."
                    },
                    2: {
                        icon: "🔥",
                        title: "Обратный выброс",
                        color: "#ff922b",
                        desc: "Вы получаете половину урона. Оружие заклинило."
                    },
                    3: {icon: "💨", title: "Забитый ствол", color: "#adb5bd", desc: "Требуется действие на починку."},
                    4: {
                        icon: "⚙️",
                        title: "Частичная осечка",
                        color: "#74c0fc",
                        desc: "Атака с помехой, урон половинный."
                    },
                    5: {
                        icon: "⚡",
                        title: "Нестабильный выстрел",
                        color: "#ffd43b",
                        desc: "Атака проходит, урон уменьшается вдвое."
                    },
                    6: {
                        icon: "😈",
                        title: "Удачный срыв",
                        color: "#c77dff",
                        desc: "Атака проходит. Следующая атака с помехой."
                    },
                };
                ...
                ChatMessage.create({
                    content: `
        <div style="background:#1a1a1a; border:1px solid #444; border-radius:10px; padding:12px; color:#f1f3f5;">
          <div style="font-size:1.05em; margin-bottom:6px;">${mishap.icon} <b style="color:${mishap.color};">Осечка: ${mishap.title}</b></div>
          <div style="background:#111; padding:6px; border-radius:6px; margin-bottom:6px; font-size:0.9em; color:#ccc;">🎲 Результат d6: <strong>${m}</strong></div>
          <div style="color:#ddd;">${mishap.desc}</div>
                ...
                if (m === 1) {
                    const dmg = await rollAndSend("1d4", actor.getRollData(), "💣 Взрыв: вы получаете урон, оружие ломается");
                    await MidiQOL.applyTokenDamage([{
                        damage: dmg.total,
                        type: "piercing"
                    }], dmg.total, new Set([token]));
                    weaponJammed = true;
                    break outer;
                } else if (m === 2) {
                    const dmg = await rollAndSend("1d4", actor.getRollData(), "🔥 Обратный выброс: вы получаете половину урона, оружие заклинило");
                    const half = Math.max(1, Math.floor(dmg.total / 2 / 2));
                    ...
                    const hit = disAtk.total >= ac || isCritM4;

                    await processHit(hit, isCritM4, entry.target, useAshBarrel, true); // true = рубим урон пополам
                } else if (m === 5) {
                    // Урон уменьшается вдвое (Автопопадание)
                    await processHit(true, false, entry.target, useAshBarrel, true); // true = рубим урон пополам
                } else if (m === 6) {
                    // Атака проходит, след. с помехой (Автопопадание)
                    disadvantageNext = true;
                    await processHit(true, false, entry.target, useAshBarrel, false); // Урон нормальный
                }
                continue; // Осечка отработана
            }
            ...
            const ac = entry.target.actor.system.attributes.ac.value;
            await processHit(attack.total >= ac || isCrit, isCrit, entry.target, useAshBarrel, false);
        }
    }

// ============================================================
// 🎯 ПОПАДАНИЕ / ПРОМАХ
// halveDamage — параметр для осечек 4 и 5
// ============================================================
async function processHit(hit, isCrit, target, ashEnabled, halveDamage = false) {
    if (!hit) {
        ChatMessage.create({
            content: `<div style="background:#1a1a1a; border:1px solid #444; padding:8px; border-radius:8px; color:#f1f3f5;">❌ Промах по <b>${target.name}</b></div>`
        });
        return;
    }

    if (typeof Sequence !== "undefined") {
        new Sequence().effect().file("jb2a.impact.004.yellow").atLocation(target).scale(0.6).play();
    }

    const baseFormula = isCrit ? "2d4 + @abilities.dex.mod" : "1d4 + @abilities.dex.mod";
    const baseRoll = await rollAndSend(baseFormula, actor.getRollData(), isCrit ? `💥 Крит по ${target.name}` : `🎯 Урон по ${target.name}`);

    let ashRoll = null;
    let applyAsh = ashEnabled && !ashUsedThisTurn;

    if (applyAsh) {
        ashUsedThisTurn = true; // Сработало! Больше в этом раунде пепел не активируется
    ...
    let totalDmg = baseRoll.total + (ashRoll?.total ?? 0);
    ...
    // Красивое предупреждение, если урон был порезан
    const halvedNote = halveDamage ? `<br><span style="color:#74c0fc; font-size:0.95em;">(Урон уменьшен вдвое из-за осечки)</span>` : "";

    ChatMessage.create({
        content: `
    <div style="background:#1a1a1a; border:1px solid #5a3a1a; padding:10px; border-radius:10px; color:#f1f3f5;">
      <div style="font-size:1.05em; margin-bottom:6px;">
        ${isCrit ? "💥 <b>КРИТИЧЕСКОЕ!</b>" : "Попадание"} → <b>${target.name}</b>
      </div>
      <div style="background:#111; padding:6px; border-radius:6px; margin-bottom:6px; font-size:0.9em; color:#ccc;">
        🎲 Основа: [${baseDice}]${ashDice ? `<br>🔥 Пепел: [${ashDice}]` : ""}${halvedNote}
      </div>
